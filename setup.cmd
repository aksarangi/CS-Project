...
if not exist "backend\main.py" (
    echo %STEP% Creating starter backend\main.py...
    (
        echo from flask import Flask, jsonify
        echo import mysql.connector, os
        ...
        echo @app.route('/')
        echo def index():
        echo.    return jsonify({"message": "Bookshop API Backend Active"})
        echo
        echo if __name__ == '__main__':
        echo.    app.run(debug=True)
    ) > backend\main.py
    echo %OK% backend\main.py created.
) else (
    echo %INFO% backend\main.py already exists — skipping.
)
echo.
